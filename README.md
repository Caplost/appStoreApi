...
# 1.快速创建代码仓库请使用下方命令
```
sudo docker run --rm -v $(pwd): $(pwd) -w  $(pwd) -e ICODE=xxxxxx cap1573/cap-tool createApi git.imooc.com/cap1573/appStoreApi
...
5.这里git.imooc.com/cap1573/appStoreApi 仓库名字需要和 go mod 名称一致
```
...
make build
```
代码执行后会产生 appStoreApi 二进制文件

## 4.编译执行二进制文件
```
make docker
```
编译成功后会自动生成 appStoreApi:latest 镜像
可使用 docker images |grep appStoreApi 查看是否生成
